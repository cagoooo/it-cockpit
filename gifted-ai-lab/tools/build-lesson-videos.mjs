...
const outputRoot = path.join(labDir, 'lesson-videos');
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'gifted-lesson-videos-'));
const tempProfile = fs.mkdtempSync(path.join(os.tmpdir(), 'gifted-lesson-videos-profile-'));
...
</style></head><body><main><header class="top"><div><div class="brand">石門智繪客 · W${code}</div><h1>${escapeHtml(title)}</h1></div><div class="badge">給三年級</div></header><div class="visual"><img src="${imageUrl}" alt=""></div><section class="message"><div class="tag">${escapeHtml(tag)}</div><p>${escapeHtml(text)}</p></section><div class="footer">阿凱老師｜桃園市龍潭區石門國民小學</div></main></body></html>`;
}

function loadPreviousManifest() {
  const manifestPath = path.join(labDir, 'lesson-video-manifest.json');
...
function writeDataFiles(manifest, mcpRun) {
  fs.writeFileSync(path.join(labDir, 'lesson-video-manifest.json'), `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
  fs.writeFileSync(path.join(outputRoot, 'notebooklm-mcp-run.json'), `${JSON.stringify(mcpRun, null, 2)}\n`, 'utf8');
  fs.writeFileSync(path.join(labDir, 'lesson-video-data.js'), `window.GIFTED_LESSON_VIDEOS = ${JSON.stringify(manifest, null, 2)};\n`, 'utf8');
}
...
try {
  for (const item of selectedWeeks) {
    ...
    const outputPath = path.join(outDir, 'lesson-video.mp4');
    run('ffmpeg', [
    ...
    manifest.items[String(item.week)] = {
      ...
      description: `lesson-videos/week-${code}/description.txt`,
...
console.log(`完成 ${selectedWeeks.length} 支三年級好懂版課前影片與教材。`);
