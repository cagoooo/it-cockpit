...
const core=['index.html','week-data.js','week-student-language.js','week-cockpit.js','week-cockpit-rich.css','week-learning-depth.js','week-phase-six.js','notebooklm-pdf-player.css','notebooklm-pdf-player.js','vendor/pdfjs/pdf.min.mjs','vendor/pdfjs/pdf.worker.min.mjs','picture-book-artifacts.json','notebook-teacher-avatar-slides.json','reports.html','reports.js','sync-status.html','sync-status.js','materials/teacher-guide.pdf','materials/student-workbook.pdf','shorts-k-manifest.json','shorts-k-data.js','shorts-k/notebooklm-mcp-run.json'];
const records=[];
...
const weeks=sandbox.window.GIFTED_WEEKS.map(item=>{
  ...
  for(const name of ['short.mp4','captions.srt','captions.vtt','captions.ass','cover.webp','transcript.txt','description.txt','upload-checklist.md']){const file=`shorts-k/week-${code}/${name}`;if(fs.existsSync(path.join(labDir,file)))records.push({file,sha256:hash(file),bytes:size(file),scope:`week-${code}`});}
  const student=notebook.find(row=>row.week===item.week)||{};
  const deep=depth.find(row=>row.week===item.week)||{};
  return{week:item.week,date:item.date,teacher:'黃凱揚老師',sessions:2,minutes:90,title:item.title,goal:item.goal,output:item.output,plan_source:'materials/teacher-guide.pdf',notebook_id:student.notebook_id||'',student_source_id:student.student_source_id||'',student_infographic_id:student.infographic_id||'',student_infographic_status:student.infographic_status||'missing',depth_source_id:deep.source_id||'',shorts_k_source:'shorts-k-manifest.json',shorts_k_format:'9:16',shorts_k_status:fs.existsSync(path.join(labDir,'shorts-k',`week-${code}`,'short.mp4'))?'ready':'missing',status:student.notebook_id&&student.student_source_id?'ready':'needs_attention'};
});
const manifest={schema_version:1,release:'2026.08.29-shorts-k-v1',generated_at:new Date().toISOString(),formal_plan:{teacher:'黃凱揚老師',school:'桃園市龍潭區石門國民小學',school_year:'115',weeks:10,sessions:20,minutes_per_session:45,source:'materials/teacher-guide.pdf'},weeks,files:records};
fs.writeFileSync(path.join(labDir,'course-version-manifest.json'),`${JSON.stringify(manifest,null,2)}\n`,'utf8');
const lines=['# 石門智繪客教材版本對照表','',`- 發布版本：${manifest.release}`,`- 產生時間：${manifest.generated_at}`,'- 正式計畫來源：`materials/teacher-guide.pdf`','','| 週次 | 日期 | 正式課程目標 | 網站與 NotebookLM | 兒童版資訊圖 | ShortsK 直式片 |','|---|---|---|---|---|---|',...weeks.map(row=>`| W${String(row.week).padStart(2,'0')} | ${row.date} | ${row.goal} | ${row.status==='ready'?'一致':'需檢查'} | ${row.student_infographic_status} | ${row.shorts_k_status} |`),'','## 同步規則','','1. 修改課程資料後，先重建週次頁面與教材。','2. 執行 `node gifted-ai-lab/tools/build-course-version-manifest.mjs` 更新雜湊。','3. 執行 `node gifted-ai-lab/tools/check-course-versions.mjs` 驗證所有檔案未漂移。','4. NotebookLM 產出仍需教師確認，未完成時保留網站自製圖卡。','5. ShortsK 影片只由教師手動上傳並設為不公開；網址填入 `shorts-k-manifest.json` 後再重建資料。'];
