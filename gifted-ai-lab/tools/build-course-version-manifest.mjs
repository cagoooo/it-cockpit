...
const weeks=sandbox.window.GIFTED_WEEKS.map(item=>{
  ...
  for(const name of ['lesson-video.mp4','captions.srt','captions.vtt','captions.ass','cover.webp','transcript.txt','description.txt']){const file=`lesson-videos/week-${code}/${name}`;if(fs.existsSync(path.join(labDir,file)))records.push({file,sha256:hash(file),bytes:size(file),scope:`week-${code}`});}
...
const manifest={schema_version:1,release:'2026.08.29-lesson-videos-v1',generated_at:new Date().toISOString(),formal_plan:{teacher:'黃凱揚老師',school:'桃園市龍潭區石門國民小學',school_year:'115',weeks:10,sessions:20,minutes_per_session:45,source:'materials/teacher-guide.pdf'},weeks,files:records};
