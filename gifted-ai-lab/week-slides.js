(function(){
  ...
  const slides=[
    {title:'課程開場',html:`<section class="slide cover"><div class="cover-copy"><div class="code">WEEK ${ww} · ${esc(data.date)} · 90 MIN</div><h1>${esc(data.title)}</h1><p class="lead">${esc(data.goal)}</p><span class="tag">阿凱老師｜桃園市龍潭區石門國民小學</span></div><img src="../assets/gifted-lab-cover.webp" alt="資訊科技專題學習情境">${speaker('今天不只要完成任務，更要留下能支持想法的證據。')}</section>`},
